...
namespace FACS01.Utilities
{
    ...
    {
        private const string RichToolName = Logger.ToolTag + "[动画替换]" + Logger.EndTag;
        ...
        [MenuItem("FACS Utils/动画/动画替换", false, 1101)]
        private static void ShowWindow()
        {
            var window = GetWindow(typeof(AnimationSwap), false, "动画替换", true);
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField($"<color=cyan><b>动画替换</b></color>\n\n" +
                $"扫描所选动画控制器，列出其中所有动画剪辑及其使用位置，并允许您用其他动画剪辑替换它们。\n" +
                $"对所选动画控制器外部的混合树的依赖将被克隆并添加为子资产。\n", FacsGUIStyles.Helpbox);
            ...
            if (GUILayout.Button("扫描!", FacsGUIStyles.Button, GUILayout.Height(40)))
            {
                if (!RunScan()) return;
                ShowNotification(new GUIContent("扫描完成!"), 1.5f);
            }
            ...
            if (animatorHierarchy != null)
            {
                ...
                foreach (var kvp in animatorHierarchy.clipReplacers)
                {
                    ...
                    if (GUILayout.Button($"{(firsttoggle ? "无" : "全部")}", GUILayout.Width(44)))
                    {
            ...
            if (canRun && GUILayout.Button("替换!", FacsGUIStyles.Button, GUILayout.Height(40)))
            {
                RunSwap();
                RunScan();
            }
        ...
        private bool RunScan()
        {
            ...
            {
                NullVars();
                ShowNotification(new GUIContent("动画控制器\n有0个层"), 3);
                return false;
            }
        ...
        private void ProcessBlendTree(SerializedProperty motion, AnimatorControllerHierarchy.AnimatorLayer.StateMachine.State.Blend_Tree b_t)
        {
            if (animatorHierarchy.blend_treeHashes[b_t.btHash] == null)
            {
                ...
                else
                {
                    ...
                    Undo.RegisterCreatedObjectUndo(newblendTree, "动画替换");
