...
namespace FACS01.Utilities
{
    ...
    {
        private const string RichToolName = Logger.ToolTag + "[修复动画]" + Logger.EndTag;
        private const string RichAnimationSourcesName = Logger.ConceptTag + "动画源" + Logger.EndTag;
        ...
        [MenuItem("FACS Utils/修复项目/修复动画", false, 1054)]
        [MenuItem("FACS Utils/动画/修复动画", false, 1101)]
        private static void ShowWindow()
        {
            var window = (FixAnimations)GetWindow(typeof(FixAnimations), false, "修复动画", true);
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField($"<color=cyan><b>修复动画</b></color>\n\n" +
                $"扫描所选动画源内的动画，并尝试修复损坏的路径和属性，" +
            ...
            if (AFGOs.animationRoots.Count > 1 &&
                GUILayout.Button("自动检测", FacsGUIStyles.Button, GUILayout.Height(20), GUILayout.MaxWidth(windowWidth * 0.75f))) AutoDetect();
            if (AFGOs.animationSources.Any(animS => animS.Count > 1) &&
                GUILayout.Button("执行!", FacsGUIStyles.Button, GUILayout.Height(40), GUILayout.MaxWidth(windowWidth * 0.75f))) RunFix();
        ...
        private static void RunFix()
        {
            ...
            Logger.Log(RichToolName + " 完成修复动画！");
        }
        ...
        private class FixAnimationsData
        {
            ...
            internal FixAnimationsData()
            {
                ...
                foreach (var kvp in AnimsToOrigins)
                {
                    ...
                    EditorUtility.DisplayProgressBar("FACS工具 - 修复动画", "请稍候...", animClipProcess);
                    analyzingAC = kvp.Key;
                ...
                EditorUtility.DisplayProgressBar("FACS工具 - 修复动画", "请稍候...", 1);
                foreach (var modAC in addedACs) AssetDatabase.SaveAssetIfDirty(modAC);

                Results = "";
                if (Fixes.Count>0) Results += $"\n已修复{Logger.RichAnimationClip}：<b>{Fixes.Count}</b>";
                if (Brokens.Count>0) Results += $"\n有问题的{Logger.RichAnimationClip}：<b>{Brokens.Count}</b>";
                Results += "\n";

                foreach (var kvp in Fixes)
                {
                    var sb = new System.Text.StringBuilder($"{RichToolName} 修复了 {Logger.RichAnimationClip} {Logger.AssetTag}{kvp.Key.name}{Logger.EndTag}");
                ...
                {
                    var sb = new System.Text.StringBuilder($"{RichToolName} {Logger.RichAnimationClip} {Logger.AssetTag}{kvp.Key.name}{Logger.EndTag} 中的问题");
            ...
            private void SetCurveFix(string newpropertyName = null)
            {
                ...
                if (addedACs.Add(analyzingAC)) Undo.RegisterCompleteObjectUndo(analyzingAC, "修复动画");
