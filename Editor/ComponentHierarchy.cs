...
namespace FACS01.Utilities
{
    public class ComponentHierarchy
    {
        ...
        private static readonly GUIContent GO_enabled_content = new("游戏对象启用状态", EditorGUIUtility.IconContent("animationvisibilitytoggleon").image);
        ...
        public void SelectDependencies(string toolName, bool recursive = false)
        {
            ...
            if (totalNewC == 0)
            { Logger.LogWarning($"{Logger.ToolTag}[{toolName}]{Logger.EndTag} 没有额外的{Logger.TypeTag}组件{Logger.EndTag}被选中。"); return; }
            var newCstr = "";
            foreach (var kvp in totalNewCL) newCstr += $"  • {Logger.TypeTag}{kvp.Key.Name}{Logger.EndTag}: {kvp.Value}\n";
            Logger.Log($"{Logger.ToolTag}[{toolName}]{Logger.EndTag} <b>{totalNewC}</b>个额外的{Logger.TypeTag}组件{Logger.EndTag}被选中！" +
                $"{(recursive ? $" 迭代次数：{iteration_i}" : "")}\n{newCstr}");
        }
        ...
        private void DisplayAllByTypes(ref bool anyon)
        {
            if (go_enables_display)
            {
                ...
                else
                {
                    ...
                    if (GUILayout.Button("全选", GUILayout.Height(16), GUILayout.Width(28))) SetAllGOEnables(true);
                    if (GUILayout.Button("全不选", GUILayout.Height(16), GUILayout.Width(44))) SetAllGOEnables(false);
                    if (GUILayout.Button($"{(go_enables_hidden ? "显示" : "隐藏")}", GUILayout.Height(16), GUILayout.Width(44)))
                    {
            ...
            foreach (var ft in toggles_by_type.Values)
            {
                ...
                else
                {
                    ...
                    if (GUILayout.Button("全选", GUILayout.Height(16), GUILayout.Width(28))) ft.SetAllToggles(true);
                    if (GUILayout.Button("全不选", GUILayout.Height(16), GUILayout.Width(44))) ft.SetAllToggles(false);
                    if (GUILayout.Button($"{(ft.hidden ? "显示" : "隐藏")}", GUILayout.Height(16), GUILayout.Width(44)))
                    {
        ...
        public class FoldableHierarchy
        {
            ...
            public void DisplayFoldable(bool go_enables_disp)
            {
                ...
                if (GUILayout.Button("全选", GUILayout.Height(16), GUILayout.Width(26)))
                {
                    if (Event.current.control) SetAllTogglesRecursive(true, go_enables_disp);
                    else SetAllToggles(true, go_enables_disp, false);
                }
                if (GUILayout.Button("全不选", GUILayout.Height(16), GUILayout.Width(44)))
                {
