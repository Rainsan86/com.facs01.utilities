...
namespace FACS01.Utilities
{
    internal class ShaderDropDownSelector
    {
        ...
        internal void Display(GUIStyle style)
        {
            ...
            var buttonContent = new GUIContent(m_Shader != null ? $" <color=orange>{m_Shader.name}</color>" : " 未选择着色器");
        ...
        private class CustomAdvancedDropdown : AdvancedDropdown
        {
            ...
            {
                var root = new AdvancedDropdownItem("着色器");
                ...
                notSupportedList.ForEach(s => AddShaderToMenu("不支持/", root, s, "不支持/" + s));
                failedCompilationList.ForEach(s => AddShaderToMenu("编译失败/", root, s, "编译失败/" + s));
