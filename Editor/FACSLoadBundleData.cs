...
namespace FACS01.Utilities
{
    ...
    {
        private const string RichToolName = Logger.ToolTag + "[资源包加载器]" + Logger.EndTag;
        ...
        internal class LoadBundleEntry
        {
            ...
            internal bool ToggleAsset(int i)
            {
                if (loadingAsset == -1)
                {
                    ...
                    if (isSceneBundle)
                    {
                        ...
                        else
                        {
                            ...
                            if (sceneInstance.IsValid() && sceneInstance.isLoaded)
                            {
                                ...
                                Logger.Log($"{RichToolName} {Logger.RichScene} {Logger.AssetTag}{assetNameDisplays[loadingAsset]}{Logger.EndTag} 已卸载。");
                            }
                    ...
                    else
                    {
                        ...
                        if (asset && asset is GameObject prefab)
                        {
                            ...
                            Undo.RegisterCreatedObjectUndo(instance, $"实例化 {instance.name}");
                ...
                {
                    Logger.LogWarning(RichToolName + " 无法同时从同一个" +Logger.RichAssetBundle+ "加载多个资产。");
                }
                return false;
            }
            ...
            private void UnloadSelectedAsset()
            {
                ...
                if (ao != null)
                {
                    ...
                    Logger.LogWarning($"{RichToolName} 已停止加载 {(isSceneBundle? Logger.RichScene:Logger.RichAsset)} {Logger.AssetTag}{assetNameDisplays[loadingAsset]}{Logger.EndTag}。");
                }
                if (isSceneBundle)
                {
                    foreach (var sceneIndex in loadedAssets.Keys)
                    {
                        ...
                        {
                            SceneManager.UnloadSceneAsync(sceneInstance, UnloadSceneOptions.UnloadAllEmbeddedSceneObjects);
                            Logger.Log($"{RichToolName} {Logger.RichScene} {Logger.AssetTag}{assetNameDisplays[sceneIndex]}{Logger.EndTag} 已卸载。");
                        }
                    }
                }
                else
                {
                    ...
                    instantiated.Clear();
                    if (destroyed) Logger.Log($"{RichToolName} 实例化的{Logger.RichGameObject}已从场景中移除。");
                }
            }
            ...
            internal void Unload(bool single = false)
            {
                ...
                if (process != null)
                {
                    ...
                    if (!loadedAssetBundle)
                        Logger.LogWarning($"{RichToolName} 正在中止{Logger.RichAssetBundle}加载，来源：{lastBundleSource}");
                    process = null;
                }
                if (loadedAssetBundle)
                {
                    ...
                    Logger.Log($"{RichToolName} 已卸载{Logger.RichAssetBundle}，来源：{lastBundleSource}");
                }
                lastBundleSource = "";
            ...
            internal IEnumerator BeginLoadBundle()
            {
                ...
                if (isURL)
                {
                    Logger.Log($"{RichToolName} 正在从{Logger.ConceptTag}URL{Logger.EndTag}获取{Logger.RichAssetBundle}：{lastBundleSource}");
                    progress = 0;
                    yield return null;
                    ...
                    {
                        Logger.LogError($"{RichToolName} 从{Logger.ConceptTag}URL{Logger.EndTag}获取{Logger.RichAssetBundle}时遇到错误：{lastBundleSource}\n{bundleWebRequest.error}");
                ...
                {
                    Logger.Log($"{RichToolName} 正在从{Logger.ConceptTag}本地文件{Logger.EndTag}获取{Logger.RichAssetBundle}：{lastBundleSource}");
                    progress = 0;
                    yield return null;
                    try { bundleFileRequest = AssetBundle.LoadFromFileAsync(lastBundleSource); }
                    catch (System.Exception e)
                    {
                        Logger.LogError($"{RichToolName} 从{Logger.ConceptTag}本地文件{Logger.EndTag}获取{Logger.RichAssetBundle}时遇到错误：{lastBundleSource}\n{e.Message}");
                        bundleFileRequest = null;
                    }
                    if (bundleFileRequest == null)
                    {
                ...
                {
                    Logger.LogWarning($"{RichToolName} 无效的{Logger.ConceptTag}URL{Logger.EndTag}或{Logger.ConceptTag}文件路径{Logger.EndTag}：{lastBundleSource}");
                ...
                {
                    Logger.LogError($"{RichToolName} 从{Logger.ConceptTag}{(isURL ? "URL" : "文件")}{Logger.EndTag}加载{Logger.RichAssetBundle}失败：{lastBundleSource}");
                    progress = -1; if (FACSLoadBundle.instance) FACSLoadBundle.instance.Repaint();
                    lastBundleSource = ""; yield break;
                }
                ...
                if (isSceneBundle)
                {
                    ...
                    if (assetNames.Length == 0)
                    {
                        ...
                        Logger.LogError($"{RichToolName} 该{Logger.RichScene}{Logger.RichAssetBundle}不包含场景路径：{lbs}");
                        yield break;
                    }
                }
                else
                {
                    ...
                    {
                        var lbs = lastBundleSource; Unload(true);
                        Logger.LogError($"{RichToolName} 该{Logger.RichAssetBundle}不包含资产路径：{lbs}");
                        yield break;
                    }
                }
                assetNameDisplays = new string[assetNames.Length]; System.Array.Copy(assetNames, assetNameDisplays, assetNames.Length);
                Logger.Log($"{RichToolName} <b>{assetNames.Length}</b> 个{Logger.TypeTag}{(isSceneBundle ? "场景" : "资产")}{Logger.EndTag}" +
                    $"已准备好从 {Logger.AssetTag}{(isURL ? lastBundleSource : System.IO.Path.GetFileName(lastBundleSource))}{Logger.EndTag} 加载！");
            ...
            private IEnumerator LoadAsset()
            {
                ...
                if (asset == null)
                {
                    ...
                    Logger.LogError($"{RichToolName} {Logger.RichAsset} {and} 为{Logger.ConceptTag}NULL{Logger.EndTag}\n资源包来源：{lbs}");
                    if (FACSLoadBundle.instance) FACSLoadBundle.instance.Repaint();
                    yield break;
                }
                ...
                if (FACSLoadBundle.instance) FACSLoadBundle.instance.Repaint();
                Logger.Log($"{RichToolName} {Logger.TypeTag}{asset.GetType().Name}{Logger.EndTag} {Logger.AssetTag}{assetNameDisplays[loadingAsset]}{Logger.EndTag} 已加载！");
                yield break;
            }
            ...
            private IEnumerator LoadScene()
            {
                ...
                {
                    Logger.LogWarning(RichToolName + " 无法在" + Logger.ConceptTag + "编辑模式" + Logger.EndTag +
                        "下加载场景。请进入" + Logger.ConceptTag + "播放模式" + Logger.EndTag + "。");
                    yield break;
                }
                ...
                if (!sceneInstance.IsValid())
                {
                    ...
                    var lbs = lastBundleSource; var and = assetNameDisplays[loadingAsset];
                    if (assetNames.Length == 1) Unload(true);
                    Logger.LogError($"{RichToolName} {Logger.RichScene} {and} 为{Logger.ConceptTag}NULL{Logger.EndTag}\n资源包来源：{lbs}");
                    if (FACSLoadBundle.instance) FACSLoadBundle.instance.Repaint();
                    yield break;
                }
                ...
                if (FACSLoadBundle.instance) FACSLoadBundle.instance.Repaint();
                Logger.Log($"{RichToolName} {Logger.RichScene} {Logger.AssetTag}{assetNameDisplays[loadingAsset]}{Logger.EndTag} 已加载！");
                yield break;
            }
