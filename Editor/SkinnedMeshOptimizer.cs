...
namespace FACS01.Utilities
{
    ...
    {
        private const string RichToolName = Logger.ToolTag + "[蒙皮网格优化器]" + Logger.EndTag;
        ...
        [MenuItem("FACS Utils/模型工具/蒙皮网格优化器", false, 1101)]
        private static void ShowWindow()
        {
            var window = GetWindow(typeof(SkinnedMeshOptimizer), false, "蒙皮网格优化器", true);
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField($"<color=cyan><b>蒙皮网格优化器</b></color>\n\n" +
                $"查找所选游戏对象/预制体/FBX（模型）中的所有蒙皮网格渲染器，帮助您移除混合形状和未使用的内部骨骼引用。\n" +
                $"所选游戏对象将被克隆为新的预制体资源，并嵌入新的网格文件作为子资源。\n", FacsGUIStyles.Helpbox);
            ...
            if (rootGO) { if (GUILayout.Button("扫描!", FacsGUIStyles.Button, GUILayout.Height(40))) Scan(); }
            ...
            if (newMeshes != null)
            {
                if (displayBlendShapes)
                {
                    ...
                    blendShapeFoldout = GUILayout.Toggle(blendShapeFoldout, "<color=green><b>可移除的混合形状（按网格）：</b></color>", FacsGUIStyles.Foldout, GUILayout.Height(16), GUILayout.ExpandWidth(true));
                    ...
                    if (blendShapeFoldout)
                    {
                        ...
                        foreach (var nm in newMeshes)
                        {
                            ...
                            {
                                if (GUILayout.Button("全不选", FacsGUIStyles.Button, GUILayout.Height(18), GUILayout.Width(45))) foreach (var bsdata in nm.BSData) bsdata.ApplyDelete = false;
                            }
                            else
                            {
                                if (GUILayout.Button("全选", FacsGUIStyles.Button, GUILayout.Height(18), GUILayout.Width(45))) foreach (var bsdata in nm.BSData) bsdata.ApplyDelete = true;
                            }
                            if (GUILayout.Button("仅空权重", FacsGUIStyles.Button, GUILayout.Height(18), GUILayout.Width(95)))
                            {
                        ...
                        if (GUILayout.Button("全选", FacsGUIStyles.Button, GUILayout.Height(20))) SetAllBSApply(true);
                        if (GUILayout.Button("仅空权重", FacsGUIStyles.Button, GUILayout.Height(20))) SetOnAllBSEmpies();
                        if (GUILayout.Button("全不选", FacsGUIStyles.Button, GUILayout.Height(20))) SetAllBSApply(false);
                    ...
                    EditorGUI.DrawRect(rect, GUITools.GetTintedBGColor(0.05f));
                    animationSourcesFoldout = GUILayout.Toggle(animationSourcesFoldout, "<color=green><b>保留来自动画源的混合形状：</b></color>", FacsGUIStyles.Foldout, GUILayout.Height(16), GUILayout.ExpandWidth(true));
                    EditorGUILayout.EndHorizontal();
                    ...
                    if (animationSourcesFoldout)
                    {
                        ...
                        if (AFGOs.animationRoots.Count > 1)
                        {
                            ...
                            if (GUILayout.Button("自动检测", FacsGUIStyles.Button, GUILayout.Height(20))) AutoDetect();
                            if (AFGOs.animationSources.Any(animS => animS.Count > 1) &&
                                GUILayout.Button("取消选择混合形状", FacsGUIStyles.Button, GUILayout.Height(20))) DeselectBlendshapes();
                ...
                {
                    EditorGUILayout.LabelField($"<color=green><b>没有可优化的混合形状</b></color>", FacsGUIStyles.Helpbox);
                }
                EditorGUILayout.Space(10);
                GUITools.ColoredToggle(ref addNonOptimized, "将未优化的网格添加到预制体", GUILayout.Height(22));
                if (GUILayout.Button("执行!", FacsGUIStyles.Button, GUILayout.Height(40))) Run();
            }
        ...
        private void Scan()
        {
            ...
            if (SMRs.Length == 0) { ShowNotification(new GUIContent("未找到蒙皮网格渲染器！")); return; }
            if (SMRs.All(smr => !smr.sharedMesh)) { ShowNotification(new GUIContent("蒙皮网格渲染器中\n没有网格？")); return; }
        ...
        {
            if (!AFGOs.AutoDetect(RichToolName)) ShowNotification(new GUIContent("没有动画根\n包含动画源"));
        }
        ...
        private void Run()
        {
            ...
            foreach (var nm in newMeshes)
            {
                EditorUtility.DisplayProgressBar("FACS工具 - 蒙皮网格优化器", $"正在优化 \"{nm.oldMesh.name}\"...", meshN / totalMeshes);
                meshN++;
                OptimizeMesh(nm);
            }
            ...
            {
                ShowNotification(new GUIContent("所有网格\n均已优化！"));
        ...
        private void DeselectBlendshapes()
        {
            ...
            if (deselCount != 0) Logger.Log($"{RichToolName} 取消选择了 <b>{deselCount}</b> 个混合形状！");
            else
            {
                if (anyBSfound) Logger.Log($"{RichToolName} 没有额外的混合形状被取消选择。");
                else
                {
                    ShowNotification(new GUIContent("没有找到动画中\n的混合形状？"));
                    Logger.LogWarning($"{RichToolName} 在动画源中未找到所选网格的混合形状。");
                }
            }
        }
        ...
        private void GenerateReport(GameObject newPrefab)
        {
            ...
            sb.Append($"\n<b>新预制体：<color=cyan>{newPrefab.name}</color></b>\n");
            sb.Append($"<b><color=green>已优化网格总数：</color> {totalOptimizedMeshes}</b> <color=green>共</color> <b>{newMeshes.Length}</b>\n");
            sb.Append($"\n <color=green>已优化网格（<b>骨骼引用</b>）：</color> <b>{optimizedBones.Count}</b>\n");
            ...
            sb.Append($"\n <color=green>已优化网格（<b>混合形状</b>）：</color> <b>{optimizedBSs.Count}</b>\n");
            ...
            Logger.Log($"{RichToolName} 完成优化 <b>{totalOptimizedMeshes}</b> 个网格！ " +
                $"<b>{totalRMVBoneRefs}</b> (<b>{(totalRMVBoneRefs * 100f / oldTotalBoneRefs):0.0}%</b>) 个骨骼引用已移除，" +
                $"<b>{totalRMVBSs}</b> (<b>{(totalRMVBSs * 100f / oldTotalBSs):0.0}%</b>) 个混合形状已移除！", newPrefab);
        }
