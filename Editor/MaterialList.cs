...
namespace FACS01.Utilities
{
    internal class MaterialList : EditorWindow
    {
        ...
        [MenuItem("FACS Utils/着色器+材质/材质列表", false, 1101)]
        private static void ShowWindow()
        {
            var window = GetWindow(typeof(MaterialList), false, "材质列表", true);
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField($"<color=cyan><b>材质列表</b></color>\n\n" +
                $"扫描选中的游戏对象及其子对象，列出所有附加的材质，" +
                $"并允许您选择它们进行编辑和在项目文件夹中定位。\n", FacsGUIStyles.Helpbox);
            ...
            if (source && GUILayout.Button("扫描!", FacsGUIStyles.Button, GUILayout.Height(40))) GetAvailableMaterials();
            if (materials == null) return;

            EditorGUILayout.LabelField($"<color=green><b>{shaderGroups.Length} 个着色器，{materials.Length} 个材质：</b></color>", FacsGUIStyles.Helpbox);
            ...
            if (GUILayout.Button("复制到剪贴板", FacsGUIStyles.Button, GUILayout.Height(40)))
            {
                var clipb = $"{source.name} 的着色器：";
                foreach (var shaderG in shaderGroups)
                {
        ...
        private void GetAvailableMaterials()
        {
            ...
            if (renderers == null || renderers.Length == 0) { ShowNotification(new GUIContent("未找到渲染器!")); return; }
            ...
            if (matDict.Count == 0) { ShowNotification(new GUIContent("未找到材质!")); return; }
