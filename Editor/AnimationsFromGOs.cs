...
namespace FACS01.Utilities
{
    ...
    {
        private const string RichAnimationRootName = Logger.ConceptTag + "动画根" + Logger.EndTag;
        ...
        public void OnGUI(FACSGUIStyles FacsGUIStyles, float windowWidth)
        {
            ...
            EditorGUILayout.LabelField("动画根", FacsGUIStyles.Helpbox, GUILayout.MaxWidth(windowWidthHalf));
            EditorGUILayout.LabelField("动画源", FacsGUIStyles.Helpbox, GUILayout.MaxWidth(windowWidthHalf));
        ...
        public bool AutoDetect(string toolName = null)
        {
            if (ComponentDependencies.TryGetAssetsAndSources(animationRoots, AnimationSourceTypes, out List<GameObject> roots, out List<List<Object>> sources))
            {
                ...
                animationRoots.Add(null); animationSources.Add(new() { null });
                if (!string.IsNullOrEmpty(toolName)) Logger.Log(toolName + " 自动检测完成！");
                return true;
            }
            if (!string.IsNullOrEmpty(toolName)) Logger.LogWarning($"{toolName} 没有{RichAnimationRootName}包含{RichAnimationSourcesName}。");
            return false;
        }
