...
namespace FACS01.Utilities
{
    ...
    {
        private const string RichToolName = Logger.ToolTag + "[组件依赖]" + Logger.EndTag;
        ...
        private static void GenerateORFH()
        {
            ...
            if (debug) Debug.Log($"[组件依赖] 组件类型扫描完成！\n" +
                $"含对象引用的组件：{componentsWithOR.Count}\n" +
                $"含未知对象引用的组件：{componentsWithUOR.Count}\n");
            debug = false;
        }
